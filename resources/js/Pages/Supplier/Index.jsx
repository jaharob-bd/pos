...
export default function Index({ auth, suppliers }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (allFieldsFilled) {
            if (isEditMode) {
                router.post(`/supplier-update/${currentSupplierId}`, data, {
                    onSuccess: () => {
                        reset();
                        setIsOpenModal(false);
                        setIsEditMode(false);
    ...
    return (
        <AuthenticatedLayout user={auth.user} header={'Supplier List'}>
            <Head title="Supplier" />
            <List openModal={openModal} suppliers={suppliers} />
            <Modal show={isOpenModal} title={isEditMode ? 'Edit Supplier' : 'Create New Supplier'} maxWidth='2xl' onClose={closeModal}>
                <div>
                    <form onSubmit={handleSubmit} className="p-2 mx-auto">
                        <div className="grid grid-cols-1 gap-1 sm:grid-cols-2">
                            ...
                            <div>
                                <label className="dark:text-gray-800" htmlFor="contact_person">Contact Person <span className="text-red-600">*</span></label>
                                <input
                                    name="contact_person"
                                    type="text"
                            ...
                            <div>
                                <label className="dark:text-gray-800" htmlFor="address">Address <span className="text-red-600">*</span></label>
                                <textarea
                                    name="address"
                                    className="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring"
                                    value={data.address}
                            ...
                            <div>
                                <label className="dark:text-gray-800" htmlFor="status">Status</label>
                                <select
                                    name="status"
                                    className="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring"
                                    value={data.status}
